...
def _fetch_summary_from_api(text: str) -> Optional[str]:
    ...
    prompt = (
        f"Кратко выдели пунктами темы, которые обсуждались в тексте:\n{text}. Ответ составь в виде списка ключевых тем объёмом не более 500 символов.  Дополнительно требуется разметка текста для корректного отображения списка в сообщении Telegram."
    )
    payload = {
